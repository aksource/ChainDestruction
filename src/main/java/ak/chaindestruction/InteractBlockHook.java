...
public class InteractBlockHook {
     ...
     */
    public static boolean destroyBlockAtPosition(@Nonnull World world, @Nonnull EntityPlayer player, @Nonnull BlockPos blockPos, @Nonnull ItemStack item) {
        ...
        boolean blockDestroyed = itemStack.getItem().onBlockDestroyed(itemStack, world, state, blockPos, player);
        if (!startBreakingBlock && blockDestroyed) {
            if (world.setBlockToAir(blockPos)) {
                ...
                state.getBlock().harvestBlock(world, player, new BlockPos(player.posX, player.posY, player.posZ), state, null, itemStack);
                ...
                if (EnchantmentHelper.getEnchantmentLevel(Enchantments.SILK_TOUCH, itemStack) == 0) {
                    int exp = state.getBlock().getExpDrop(state, world, blockPos, EnchantmentHelper.getEnchantmentLevel(Enchantments.FORTUNE, itemStack));
     ...
     */
    private static void dropItemNearPlayer(@Nonnull World world, @Nonnull EntityPlayer player, @Nonnull BlockPos blockPos) {
     ...
     */
    public static void destroyItem(@Nonnull EntityPlayer player, @Nonnull ItemStack item, boolean isInMultiTool, IInventory tools, int slotNum) {
        if (isInMultiTool) {
     ...
     */
    public void doMouseEvent(@Nonnull ItemStack item, @Nonnull EntityPlayer player, byte mouse, boolean isFocusObject) {
        try {
            ICDPlayerStatusHandler status = CDPlayerStatus.get(player);
     ...
     * @return 破壊対象かどうか
     */
    private boolean checkBlockValidate(@Nonnull EntityPlayer player, @Nonnull IBlockState state, @Nonnull ItemStack heldItem) {
        ICDPlayerStatusHandler status = CDPlayerStatus.get(player);
        if (heldItem.isEmpty()) {
            return false;
        }
     ...
     */
    private void addAndRemoveBlocks(@Nonnull Set<String> set, @Nonnull EntityPlayer player, @Nonnull IBlockState state) {
     ...
     */
    private boolean isChainDestructionActionable(@Nonnull EntityPlayer player, @Nonnull IBlockState state, @Nonnull ItemStack heldItem) {
        ICDPlayerStatusHandler status = CDPlayerStatus.get(player);
     ...
     */
    @Nonnull
    private Set<BlockPos> searchBlock(@Nonnull World world, @Nonnull ICDPlayerStatusHandler status, @Nonnull EntityPlayer player, @Nonnull IBlockState target, @Nonnull BlockPos targetPos) {
     ...
     */
    @Nonnull
    private LinkedHashSet<BlockPos> getConnectedBlockSet(@Nonnull ICDPlayerStatusHandler status, @Nonnull BlockPos origin, @Nonnull Set<BlockPos> searchedBlockSet) {
     ...
     */
    private void destroyBlock(@Nonnull World world, @Nonnull EntityPlayer player, @Nonnull ItemStack item, @Nonnull LinkedHashSet<BlockPos> connectedBlockSet) {
     ...
     * @return 同種ならtrue
     */
    private boolean checkBlock(@Nonnull ICDPlayerStatusHandler status, @Nonnull IBlockState target, @Nonnull IBlockState check, @Nonnull ItemStack heldItem) {
