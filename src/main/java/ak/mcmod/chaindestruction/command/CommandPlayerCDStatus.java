...
public class CommandPlayerCDStatus {
  ...
  public static void register(final CommandDispatcher<CommandSourceStack> commandDispatcher) {
    commandDispatcher.register(Commands.literal(COMMAND_PLAYER_CD_STATUS).requires(e -> e.hasPermission(2))
            ...
            .then(Commands.literal("add_forbidden").then(Commands.argument("tag", ComponentArgument.textComponent())
                            .executes(e -> addForbiddenTag(e.getSource(), null, ComponentArgument.getComponent(e, "tag").getContents())))
                    .then(Commands.argument("target", EntityArgument.player())
                            .executes(e -> addForbiddenTag(e.getSource(), EntityArgument.getPlayer(e, "target"), ComponentArgument.getComponent(e, "tag").getContents()))))
            .then(Commands.literal("remove_forbidden").then(Commands.argument("tag", ComponentArgument.textComponent())
                            .executes(e -> removeForbiddenTag(e.getSource(), null, ComponentArgument.getComponent(e, "tag").getContents())))
                    .then(Commands.argument("target", EntityArgument.player())
                            .executes(e -> removeForbiddenTag(e.getSource(), EntityArgument.getPlayer(e, "target"), ComponentArgument.getComponent(e, "tag").getContents())))));
  }
  ...
  private static int reset(CommandSourceStack commandSource, @Nullable Player playerEntity) {
    ...
    if (Objects.nonNull(playerEntity)) {
      ...
      commandSource.sendSuccess(new TranslatableComponent(COMMAND_PLAYER_STATUS_RESET_SUCCESS), true);
    } else {
      return 1;
    }
    return 0;
  ...
  private static int addForbiddenTag(CommandSourceStack commandSource, @Nullable Player playerEntity, String forbiddenTag) {
    if (Objects.isNull(playerEntity)) {
      try {
        playerEntity = commandSource.getPlayerOrException();
      } catch (CommandSyntaxException e) {
    ...
    if (Objects.nonNull(playerEntity)) {
      playerEntity.getCapability(CapabilityAdditionalPlayerStatus.CAPABILITY).ifPresent(status ->
              status.getForbiddenTags().add(forbiddenTag));
      commandSource.sendSuccess(new TranslatableComponent(COMMAND_PLAYER_STATUS_ADD_FORBIDDEN_TAG_SUCCESS, forbiddenTag), true);
    } else {
      return 1;
    }
    return 0;
  ...
  private static int removeForbiddenTag(CommandSourceStack commandSource, @Nullable Player playerEntity, String forbiddenTag) {
    if (Objects.isNull(playerEntity)) {
      try {
        playerEntity = commandSource.getPlayerOrException();
      } catch (CommandSyntaxException e) {
    ...
    //noinspection ConstantConditions
    if (Objects.nonNull(playerEntity)) {
      playerEntity.getCapability(CapabilityAdditionalPlayerStatus.CAPABILITY).ifPresent(status -> status.getForbiddenTags().remove(forbiddenTag));
      commandSource.sendSuccess(new TranslatableComponent(COMMAND_PLAYER_STATUS_REMOVE_FORBIDDEN_TAG_SUCCESS, forbiddenTag), true);
    } else {
      return 1;
    }
    return 0;
