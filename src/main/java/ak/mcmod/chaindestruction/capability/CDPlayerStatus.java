...
public class CDPlayerStatus implements ICDPlayerStatusHandler,
  ...
  public BlockPos getMinPos(Entity entity, BlockPos targetPos) {
    ...
    if (isDigUnder()) {
      ...
      y = Math.max(ak.mcmod.chaindestruction.api.Constants.MIN_Y, MathHelper.floor(entity.position().y));
    } else if (maxDestroyedBlock > 0) {
      y = Math.max(ak.mcmod.chaindestruction.api.Constants.MIN_Y, MathHelper.floor(entity.position().y) - 1);
    }
    return new BlockPos(
